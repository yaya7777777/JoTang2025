...
![alt text](5550edfa576cf2338f0f1770ab317d8a.jpg)
...
[三小时深度学习PyTorch快速入门！](https://www.bilibili.com/video/BV1cPSZYcEtn/?spm_id_from=333.337.search-card.all.click&vd_source=1cf9ae4d63ff94aa45638c152e755eab)

先建立一个数据文库moons_loader
...
然后对数据进行划分，网上说只用化为训练集和验证集，但为了完整性，这里我们还是 **假意** 设置了一个测试集
...
torch.max是取维度1上最大值的索引，即预测类别（维度0是batch，dim是维度，[1]指获得的是索引而非值）

统计预测正确的样本数，.item是只读取标量数据，避免环境变量影响

![alt text](a6727fa4476dbdcc59ce6abc807b4800.png)

最开始我跟着视频学习，并进行模仿，but我发现他把训练数据放在了主函数，而我需要的是一个训练模型的函数
...
紧接着把我们需要的数据计算并返回一下，包括准确率、损失，然后更新进度

（更新进度这里是在pytorch网站上抄的，理解一下）

![alt text](66b27171ca751ece125c4eb50bdbf004.png)

可视化的过程就没什么好说的了，就是一整个复制粘贴的大动作，除了label，title全是copy的
